...
class Loader {
	...
	public function add_install_service_hooks() {
		...
		if ( false === get_option( 'sgo_install_service_7_3_0', false ) ) {
	...
	public function add_config_hooks() {
		...
		foreach ( $this->config->config_options as $option ) {
			add_action( 'update_option_siteground_optimizer_' . $option, array( $this->config, 'update_config' ) );
		}
	}
}
